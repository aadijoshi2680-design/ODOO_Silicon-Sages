...
## Agent Implementation Briefing

> A complete description of every feature built — what it is, what it does, and how it works

### Legend

| Icon | Meaning |
|------|---------|
| 📋 Core Feature | From the original requirements |
...
> 📋 **What We Built**
> A complete multi-role auth system where signing up once auto-bootstraps a full company environment with a default currency and an Admin account ready to manage users.

### 1.1 First Signup Behaviour

When a brand-new user registers for the first time, the system does all of the following automatically in a single transaction — the user sees a ready-to-use dashboard on first login:
...
| **Default currency** | The company's base currency is set automatically from the country the user selects during signup, using the country → currency API. |
| **Admin user created** | The registering user is given the Admin role automatically. They immediately have full permissions. |

### 1.2 Admin Capabilities

After login, the Admin can fully manage the company's user structure:
...
> 📋 **What We Built**
> Employees can submit individual expense claims with rich fields, attach receipts, and view the real-time status of every claim they have ever submitted.

### 2.1 Submit an Expense Claim

The expense submission form collects the following fields:
...
Every submitted expense is visible in the employee's history view with full status tracking:
...
> ✨ **Why We Added This**
> In real-world usage employees accumulate many small expenses throughout a week — a coffee here, a cab there. Submitting each one individually creates approval queue spam.
> The Wallet is a personal ledger inside the employee account where they log expenses on the go and decide later which ones to officially submit for reimbursement.
> This was built on top of the core requirements.

### 3.1 What the Wallet Is

The Wallet is a private holding area that belongs to the employee only. Nothing in the wallet is visible to managers or admins until the employee explicitly submits it. Think of it like a digital expense envelope that the employee fills up before sealing and sending.

### 3.2 Adding an Entry to the Wallet

An employee can add a wallet entry any time — even without submitting it. The entry form collects:
...
The wallet view shows all entries as a table with these columns:
...
| **Status** | In Wallet / In Bundle / Submitted — explained below. |
| **Actions** | Edit or Delete (only available while status is 'In Wallet'). |

> A running total row at the bottom of the table shows the sum of all **In Wallet** entries converted to the company's base currency.
...
> ✨ **Why We Added This**
> Instead of raising 10 separate approval requests for 10 small expenses, the employee selects the relevant wallet entries, groups them into one Bundle, and sends the whole bundle through the approval chain in a single request.
> Approvers review one clean summary table instead of ten separate tickets.
...
| 3 | Employee | Clicks 'Create Bundle'. A modal opens asking for a Bundle Name (required) and a Bundle Note (optional overall description). |
| 4 | System | Auto-generates the Bundle Summary Table (see Section 4.2) and displays it for employee review. |
...
When an employee creates a bundle or when an approver reviews one, this is the exact table both parties see:
...
| **Grand Total** | | | | | **(company currency)** | **₹ 16,020** |
...
### 4.4 Bundle Rules — Business Logic

> ⚠️ **Critical Rules the Agent Must Know**
...
> If the IS MANAGER APPROVER flag is checked on an employee's record, that employee's assigned manager is ALWAYS inserted as Step 1 in the approval chain — before any other configured approver.

### 5.1 Sequential Multi-Level Approval

Admin configures an ordered list of approvers. The system enforces this order strictly — no approver can act until the previous one has acted.
...
On top of sequential approval, Admin can attach conditional rules that can auto-approve an expense at any point in the chain when a condition is met.
...
> ⚠️ **Key Point**
> Multi-level sequential approval AND conditional rules can both be active on the same expense/bundle at the same time. The conditional rule is checked after **EVERY** approver action.
...
> 📋 **What We Built**
> When an employee uploads a receipt image or PDF (in both the individual expense form and the wallet entry form), the OCR engine reads the document and auto-fills all fields. The employee reviews and can edit anything before saving.
...
> ⚠️ **Important**
> OCR is a **helper — not a gatekeeper**. All auto-filled fields are editable. The employee is always responsible for confirming the values before submission.
...
| **Bundle table** | Each row in the Bundle Summary Table shows original currency + converted amount. Grand total is always in company currency. |
...
This is the complete journey of a reimbursement — from the employee logging an expense to the final decision. This covers both individual expenses and bundles.
...
The visual design for all screens, navigation flows, and component layouts is available at:

🔗 **Excalidraw Mockup:** [https://link.excalidraw.com/l/65VNwvy7c4X/4WSLZDTrhkA](https://link.excalidraw.com/l/65VNwvy7c4X/4WSLZDTrhkA)
